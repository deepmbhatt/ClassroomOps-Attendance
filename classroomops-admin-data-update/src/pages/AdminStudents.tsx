import { FileDown, Plus, Save, Search, Trash2, Upload, UsersRound } from 'lucide-react'
...
export function AdminStudents() {
  ...
  const importRoster = useMutation({
    mutationFn: async () => {
      if (newRows.length) await bulkCreateStudents(newRows)
    ...
    onSuccess: async (result) => {
      setMessage(`${result.created} new accounts created, ${result.updated} existing students updated.`)
      ...
      <div className="two-column import-layout">
        <Card>
          ...
          <div className="import-actions">
            <StatusPill tone={preview.errorCount ? 'danger' : 'good'}>{newRows.length} new / {existingRows.length} updates / {preview.errorCount} errors</StatusPill>
            <IconButton disabled={preview.errorCount > 0 || !validRows.length || importRoster.isPending} onClick={() => importRoster.mutate()}><Upload size={16} />Apply roster file</IconButton>
          </div>
        </Card>
