...
const adminGroups = [
  ...
  },
  {
    ...
    links: [
      ...
      ['/admin/attendance-review', 'Attendance review', ClipboardCheck],
...
export function SectionTabs<T extends string>({ value, onChange, items }: { value: T; onChange(value: T): void; items: Array<{ value: T; label: string; icon?: React.ReactNode; count?: number }> }) {
  return (
    <div className="section-tabs" role="tablist">
      {items.map((item) => (
        <button key={item.value} type="button" role="tab" title={item.label} aria-selected={value === item.value} className={value === item.value ? 'active' : ''} onClick={() => onChange(item.value)}>
...
export function IconButton({ children, className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & React.PropsWithChildren) {
  const childText = Children.toArray(children)
    ...
    .join(' ')
    .trim()
  const tooltip = (props.title ?? props['aria-label'] ?? childText) || undefined
  return <button className={`icon-text ${className}`} title={tooltip} {...props}>{children}</button>
}
