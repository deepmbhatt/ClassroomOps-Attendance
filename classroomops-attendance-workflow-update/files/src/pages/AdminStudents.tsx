import { BookOpen, CheckCircle2, FileDown, FileSpreadsheet, Save, Search, Trash2, Upload, UserRoundCheck, Users, UsersRound, XCircle } from 'lucide-react'
...
export function AdminStudents() {
  ...
  async function importCourses() {
    const [, ...rows] = parseCsv(courseCsv)
    let saved = 0
    try {
      for (const row of rows) {
        const [code = '', title = '', term = '', active = 'true'] = row.map((cell) => cell.trim())
    ...
    <>
      ...
      {workspace === 'courses' ? <div className="admin-data-grid">
        <Card>
          ...
          <p className="muted-copy">Upload the course file to stage additions or updates. Review saved courses in the sheet before using them.</p>
          <div className="format-box compact"><code>Code</code><code>Title</code><code>Term</code><code>Active</code></div>
          <label className="file-picker" title="Choose an Excel or CSV course file"><Upload size={17} />Upload courses Excel/CSV<input type="file" accept=".csv,.xlsx,.xls,text/csv" onChange={(event) => void readCourseFile(event)} /></label>
          {courseFileName ? <p className="file-summary"><FileSpreadsheet size={16} />{courseFileName}</p> : null}
          <IconButton disabled={!courseCsv} onClick={() => void importCourses()}><Upload size={16} />Import/update courses</IconButton>
        </Card>
