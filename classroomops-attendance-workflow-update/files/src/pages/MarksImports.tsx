...
export function MarksImports() {
  ...
  const students = useMemo(() => {
    const enrolledIds = new Set((data?.courseMemberships ?? [])
      .filter((membership) => membership.course_id === selectedCourse?.id && !membership.deleted_at)
      .map((membership) => membership.student_id))
    return data?.profiles
      .filter((profile) => enrolledIds.has(profile.id) && profile.role === 'student' && profile.approval_status === 'approved' && !profile.deleted_at)
      .map((profile) => ({
        student_id: profile.student_id ?? '',
        full_name: profile.full_name,
        email: profile.email,
      })) ?? []
  }, [data, selectedCourse?.id])
    ...
    <>
      ...
      <div className="workspace-panel">
        {workspace === 'import' ? <Card>
          ...
          <label className="file-picker" title="Choose an Excel or CSV marks file"><Upload size={17} />Upload marks Excel/CSV<input type="file" accept=".csv,.xlsx,.xls,text/csv" onChange={(event) => void readFile(event)} /></label>
          {fileName ? <p className="file-summary"><FileCheck2 size={16} />{fileName}</p> : null}
          {preview.rows.length ? <div className="table-scroll upload-preview"><table><thead><tr><th>Row</th><th>Student</th><th>Marks</th><th>Status</th></tr></thead><tbody>{preview.rows.map((row) => <tr key={row.rowNumber} className={row.status === 'error' ? 'error-row' : undefined}><td>{row.rowNumber}</td><td>{row.studentName ?? row.studentId}<small>{row.studentId}</small></td><td>{row.value}</td><td><StatusPill tone={row.status === 'valid' ? 'good' : 'danger'}>{row.messages.join(', ') || 'ready'}</StatusPill></td></tr>)}</tbody></table></div> : null}
