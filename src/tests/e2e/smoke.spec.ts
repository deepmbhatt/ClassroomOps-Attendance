...
test('admin dashboard is responsive and available', async ({ page }) => {
  await page.goto('/')
  await expect(page.getByRole('heading', { name: /Fundamentals of Machine Learning/i })).toBeVisible()
  await expect(page.getByText(/Face enrollments/i).first()).toBeVisible()
})
