...
const fallbackModelVersion = 'demo-deterministic-v1'
const modelVersion = modelPath ? `onnx:${modelPath.split('/').pop()}` : fallbackModelVersion
...
export async function createEmbeddingFromCanvas(canvas: HTMLCanvasElement, mode: ComputeMode): Promise<EmbeddingResult> {
  ...
  if (!modelPath) {
    throw new Error('A valid ONNX face embedding model is required before biometric processing can mark enrollments ready. Set VITE_FACE_EMBEDDING_MODEL only after adding the .onnx file under public/models/.')
  }
  ...
  try {
    ...
    const firstBytes = new TextDecoder().decode(modelBytes.slice(0, Math.min(80, modelBytes.byteLength))).trimStart()
    if (modelBytes.byteLength < 1024 || firstBytes.startsWith('<')) {
    ...
    throw new Error(`Could not load the ONNX face embedding model: ${message}`)
  }
}
